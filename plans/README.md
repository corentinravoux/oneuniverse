...
| **C2** | **Engine contracts + first plugins** — `ReconstructionEngine` (data→field) + `ForwardEngine` (field→products) ABCs; refactor Wiener as `WienerReconstruction`, linear sim as `LinearForwardEngine`. Proves generality with one engine of each role | **active** — [`plans/2026-06-02-twin-coupling-roadmap.md`](2026-06-02-twin-coupling-roadmap.md) |
| **C3** | **Validation harness** (`twin/validation.py`) — standard r(k) / power-ratio / `k_half` metrics every step reports through; regression-tested (the methods-paper spine) | planned (twin roadmap) |
| **C4** | **Mock data / selection layer** (`twin/mock_survey.py`) — synthetic mask + survey geometry + mock n(z); masked/apodised Wiener. Still no real-dataset loading | planned (twin roadmap) |
| **C5** | **Constrained realization** (Hoffman–Ribak) — Wiener mean → proper constrained IC (small-scale variance restored); the IC that feeds resimulation | planned (twin roadmap) |
| S5 | **OUF-Sim write-path optimisation + product coverage** (streaming chunker, parallel/MPI writes, ExecutionPlan enforcement, wrap-don't-re-encode, + tree/phase_space/gr_fields/checkpoints/ic_posterior products) | planned (after C5) — [`plans/2026-06-02-phaseS5-oufsim-optimisation-and-coverage.md`](2026-06-02-phaseS5-oufsim-optimisation-and-coverage.md) |
| S6 | **OUF-Sim read-path optimisation** (benchmark harness + per-lever tests: projection, predicate pushdown, index cache, parallel reads, Morton order, GPU hook) | planned (after S5) — [`plans/2026-06-02-phaseS6-oufsim-read-optimisation.md`](2026-06-02-phaseS6-oufsim-read-optimisation.md) |
| S7 | **AMR octree layout + input/IC products** (toy refinement, Morton node index, `read_amr_box`, IC product `has_input=True`) | planned (after S6) — [`plans/2026-06-02-phaseS7-amr-and-input-products.md`](2026-06-02-phaseS7-amr-and-input-products.md) |
| **S8** | **Resimulation orchestration — the digital-twin core, the big stage** (sCOLA + zoom ICs + separate-universe tides). Six sub-phases: S8.0 research ✅, S8.1 fast-PM mini-sim (= 2nd `ForwardEngine`), S8.2 far-field φ(a), S8.3 IC extraction + **Gate 1**, S8.4 COLA-frame coupling + buffers, S8.5 merge + **Gate 2/3**, S8.6 `SimDatabase` control plane. Verdict: feasible as controlled approximation, not a full-sim replacement | planned (after S7) — [`plans/2026-06-02-phaseS8-resimulation-orchestration.md`](2026-06-02-phaseS8-resimulation-orchestration.md); research [`research/2026-06-02-resimulation-orchestration-feasibility.md`](../research/2026-06-02-resimulation-orchestration-feasibility.md) |
